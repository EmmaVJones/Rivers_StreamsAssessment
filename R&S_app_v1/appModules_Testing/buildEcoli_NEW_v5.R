...
# Find any STV exceedance in any window
STVexceedance <- function(z, sampleRequirement, STV, geomeanCriteria){
  STVexceedanceDontCareAboutSampleN <- filter(z, `STV Exceedances In Window` >= 1)
  # First test if any STV exceedances
  if(nrow(STVexceedanceDontCareAboutSampleN) == 0){
    return(mutate(z, `Assessment Decision` = 'Fully Supporting'))
  } else {
    # If 2+ STV exceedances in any size window
    morethan1STVexceedanceInAnyWindow <- filter(STVexceedanceDontCareAboutSampleN, `STV Exceedances In Window` >= 2)
    if(nrow(morethan1STVexceedanceInAnyWindow) > 0){
      return(mutate(STVexceedanceDontCareAboutSampleN, `Assessment Decision` = paste('Impaired: 2 or more STV exceedances in a 90 day window')))  
    } else { 
  ...
  if(nrow(STVexceedanceDontCareAboutSampleN) > 0){ 
    ...
    if(nrow(morethan1STVexceedanceInAnyWindow) > 0){
      return(mutate(STVexceedanceDontCareAboutSampleN, `Assessment Decision` = paste('Impaired: 2 or more STV exceedances in a 90 day window')))  
    } else { # Only other option is only 1 STV exceedance in any size window
      nonOverlappingIntervalsPassingGeomeanButHave1STV <- nonOverlappingIntervals(STVexceedanceDontCareAboutSampleN, TRUE)
      if(nrow(nonOverlappingIntervalsPassingGeomeanButHave1STV) >=2){
        ...
        return(mutate(STVexceedanceDontCareAboutSampleN, `Assessment Decision` = 
                        paste('Insufficient Information: 1 STV exceedance in 90 day window (Prioritize for follow up monitoring)'))) }
      
      
    } 
  } else { # If no geomean exceedances AND no STV rate exceedances in ANY 90 day window then only option to call fully supporting
      ...
      if(nrow(geomeanApplies) > 1){
        ...
        if(nrow(geomeanExceedances) > 0){
          ...
          if(nrow(STVexceedanceAnd10Samples) > 0){
            ...
            # If no geomean exceedances and no STV exceedances in 90 day window WITH 10+ samples then need to check windows without 10+ samples
            ...
            if(nrow(STVexceedanceDontCareAboutSampleN) > 0){ 
              ...
              if(nrow(morethan1STVexceedanceInAnyWindow) > 0){
                ...
                # Only other option is only 1 STV exceedance in any size window
                nonOverlappingIntervalsPassingGeomeanButHave1STV <- nonOverlappingIntervals(STVexceedanceDontCareAboutSampleN, TRUE, last2years)
                if(nrow(nonOverlappingIntervalsPassingGeomeanButHave1STV) >=2){
                  return(mutate(nonOverlappingIntervalsPassingGeomeanButHave1STV, 
                  ...
                  return(mutate(STVexceedanceDontCareAboutSampleN, `Assessment Decision` = 
                                  paste('Insufficient Information: meets geomean criteria but 1 STV exceedance in 90 day window (Prioritize for follow up monitoring)'))) }
                
                
              } 
            } else { # If no geomean exceedances AND no STV rate exceedances in ANY 90 day window then only option to call fully supporting
              return(mutate(geomeanApplies, `Assessment Decision` = 'Fully Supporting, no geomean exceedances and no STV exceedances'))  }
          }
        }
