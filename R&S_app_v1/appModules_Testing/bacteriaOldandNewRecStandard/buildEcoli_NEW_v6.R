...
tish2 <- read_excel('exampleData/assessment_workbk_Ecoli_updated.xlsx', sheet = 'assessment') %>%
...
View(z %>% distinct(`StationID`, .keep_all = T) %>% select(`Assessment Decision`, StationID))

# Test individuals stations
...
# 6-MADEUPBRANCH
results <- bacteriaAssessmentDecision( filter(tish2, ID == "6-MADEUPBRANCH"), 
                                       sampleRequirement = 10, STV = 410, geomeanCriteria = 126) 
unique(results$`Assessment Decision`)

# 8-NOPESTREAM
results <- bacteriaAssessmentDecision( filter(tish2, ID == "8-NOPESTREAM"), 
                                       sampleRequirement = 10, STV = 410, geomeanCriteria = 126) 
unique(results$`Assessment Decision`)
...
rm(list=setdiff(ls(), c('bacteriaExceedances_NewStd', 'bacteriaAssessmentDecision',"nonOverlappingIntervals",
                        'STVexceedance_noWindowSize', 'geomeanExceedance')))
...
# Test fixed VDH data, note same logic works for entero but limits altered

VDH <- read_excel('exampleData/VDH data.xlsx', sheet = 'Sheet1') 
...
results2 <- bacteriaAssessmentDecision(filter(VDH, ID == "10TH VIEW, BEHIND QUALITY INN,1010 W OCEAN VIEW AVE"),
                                       sampleRequirement = 10, STV = 130, geomeanCriteria = 35) 
