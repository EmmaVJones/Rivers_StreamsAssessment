...
server <- function(input, output, session) {
  ...
  output$StationID <- renderUI({unique(sfLines$`Point Unique Identifier`)})
...
#test1 <- data.frame(StationID = as.character(NA), ID305B = as.character(NA)) 
...
for (i in 1:nrow(siteWithTooMany4)){
  ...
  #test1[i,] <- rbind(cbind(StationID = 'kjfdsakj', ID305B = 'kjdfsakjdsfakjdsfkjsdaf'))
    #data.frame(StationID = as.character(zz[1][[1]]), ID305B = as.character(zz[2][[1]]))
}
