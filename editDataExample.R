...
#testData <- tooMany[1:3,] %>% st_set_geometry(NULL)
testData <- tooMany%>% st_set_geometry(NULL)


ui <- fluidPage(
  uiOutput('filterData'),
  verbatimTextOutput('selectTest'),
  textInput("mydata","Enter data name",value="testData"),
  editableDTUI("table1"),
  verbatimTextOutput("test")
)
server <- function(input, output) {
  
  output$filterData <- renderUI({ selectInput('selectedStation', 'Select Station', choices = unique(testData$`Point Unique Identifier`))})
  
  selectedData <- reactive({
    req(input$selectedStation)
    filter(testData, `Point Unique Identifier` %in% input$selectedStation)
  })
  
  output$selectTest <- renderPrint({
    req(input$selectedStation)
    print(selectedData())})
  
  df=callModule(editableDT,"table1", dataname=reactive(filter(testData, `Point Unique Identifier` %in% input$selectedStation)),inputwidth=reactive(170))
...
editableDTUI <- function (id) {
  ns = NS(id)
  fluidPage(fluidRow(actionButton(ns("delRow"), "Delete", icon = icon("remove", 
                                                                      lib = "glyphicon")), actionButton(ns("addRow"), "Add New", 
                                                                                                        icon = icon("plus", lib = "glyphicon")), actionButton(ns("editData"), 
                                                                                                                                                              "Edit Data", icon = icon("wrench", lib = "glyphicon")), 
                     radioButtons3(ns("selection"), "Data Selection", choices = c("single", 
                                                                                  "multiple"), inline = TRUE, labelwidth = 150, align = "center"), 
                     p(""), DT::dataTableOutput(ns("origTable")), conditionalPanel(condition = "true==false", 
          ...
          inputwidth = reactive(100)) 
{
  ...
  editData2 = reactive({
    ...
    input$addRow
    ns <- session$ns
    showModal(modalDialog(title = "Edit Data", footer = tagList(actionButton(ns("remove"), 
                                                                             "Delete", icon = icon("remove", lib = "glyphicon")), 
                                                                actionButton(ns("update"), "Update", icon = icon("ok", 
                                                                                                                 lib = "glyphicon")), modalButton("Close", icon = icon("eject", 
